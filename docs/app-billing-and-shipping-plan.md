...
Actual app registration, issued delegation credentials, notification keys and a funded inference credential remain outstanding. Railway CLI sign-in is confirmed, but the Outreachr service has no deployment. The user designated `shawmakesmagic@gmail.com` as billing owner, and browser readback now confirms that account in the Cloud dashboard. Registration must still verify the app's organization ownership; a signed-in account alone does not assign it. The generic Cloud API integration is still undergoing central validation and is not asserted deployed here.
