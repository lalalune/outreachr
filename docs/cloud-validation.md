...
The vendored SDK is built from Cloud source `4970b47fbb83533390a515d75e719243d56d1a4f`. Delegation list, register, rotate and revoke operations now require JSON results and reject HTTP 204/205 empty responses. Compared with the package merged in [PR #50](https://github.com/lalalune/outreachr/pull/50), only the delegation runtime, its declaration and map, and the compiled HTTP server test differ. The existing generic refund `superseded` state is retained. The generated route catalog continues to exclude retired product-specific endpoints. See the adjacent vendor manifest for source and artifact hashes. The package does not establish that matching Cloud APIs are deployed.
